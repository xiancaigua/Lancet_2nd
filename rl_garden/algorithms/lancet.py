...
class Lancet(WSRL):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if residual_hidden_dim < 1 or residual_hidden_layers < 1:
            raise ValueError("residual_hidden_dim and residual_hidden_layers must be >= 1.")
        if residual_lr <= 0:
        ...
        self.residual_network: Optional[ResidualQNetwork] = None
    ...
    def _setup_model(self) -> None:
        ...
        if not isinstance(obs_space, spaces.Box) or not isinstance(action_space, spaces.Box):
            raise TypeError("Lancet v1 supports flat Box state/action observations only.")
        state_dim = int(np.prod(obs_space.shape))
        action_dim = int(np.prod(action_space.shape))
        self.residual_network = ResidualQNetwork(
            state_dim,
            action_dim,
    ...
    def _extra_checkpoint_state(self) -> dict[str, Any]:
        ...
        state["lancet"] = {
            ...
            "residual_network": (
                self.residual_network.state_dict() if self.residual_network is not None else None
            ),
        }
        return state
    ...
    def residual(self, obs: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        ...
        if self.residual_network is None:
            return torch.zeros((actions.shape[0], 1), device=actions.device, dtype=actions.dtype)
        if not isinstance(obs, torch.Tensor):
            raise TypeError("Lancet v1 residual expects flat tensor state observations.")
        return self.residual_network(obs, actions)

    def corrected_q_values(self, obs: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        """Return all base ensemble Qs plus the shared residual correction."""
        return self._critic_forward(obs, actions, target=False) + self.residual(obs, actions).unsqueeze(0)
    ...
    def _u_variation_loss(self, data: Any) -> torch.Tensor:
        ...
        if self.lambda_u_variation != 0:
            raise RuntimeError("Lancet U-variation loss has no approved definition yet.")
        return torch.zeros((), device=self.device)

    def _post_critic_update(self, data: Any, critic_info: dict[str, torch.Tensor]) -> None:
        super()._post_critic_update(data, critic_info)
        if self.residual_network is None or self.residual_optimizer is None:
            return
        with torch.no_grad():
            base_q = self._critic_forward(data.obs, data.actions, target=False).mean(dim=0)
