"""SPOT offline pretraining registration.

Runs VAE pretraining (``agent.pretrain_vae()``) once, right after the dataset
is loaded and before the main TD3 gradient loop starts -- mirroring
``fit_obs_normalizer()``'s existing one-time-setup hook in ``_runner.py``.
"""
...
class SPOTArgs(
    ...
    """SPOT offline pretraining. Box or Dict (vision) observations."""
...
def build_spot(args, env_spec, logger, eval_env=None):
    from rl_garden.algorithms import SPOT
    from rl_garden.training.inspection import construct_agent

    return construct_agent(SPOT, **_spot_kwargs(args, env_spec, logger, eval_env))


def run_spot(args: SPOTArgs) -> None:
    from rl_garden.training.offline._runner import run_offline

    run_offline(args, build_agent=build_spot)




def _spot_algorithm_cls() -> type:
    from rl_garden.algorithms import SPOT

    return SPOT

registry.register("spot", SPOTArgs, run_spot, algorithm_cls=_spot_algorithm_cls)
