"""OPAL VAE pretraining run function.
...
``OPAL`` has no replay buffer -- its dataset is a ``WindowedTrajectoryDataset``
loaded directly in the constructor. Mirrors ``hilp.py``'s bespoke
...
def _run_opal(args: OPALArgs, cleanup: list[Callable[[], None]]) -> None:
    ...
    if not has_config_session():
        normalized_args, preflight = prepare_standalone(
            args, registry=registry, training_phase="offline", algorithm="opal"
        )
        with config_session(preflight, dry_run=False):
            return _run_opal(normalized_args, cleanup)
    ...
    run_name = args.exp_name or f"opal__{args.seed}__{int(time.time())}"
    checkpoint_dir = None
    ...
    if dry_run:
        ...
        logger = Logger.create(
            ...
            log_group=args.log_group or "opal",
        )
    ...
    if args.load_checkpoint is not None:
        ...
        if args.std_log:
            print(f"[opal] resumed_from={args.load_checkpoint}", flush=True)
    ...
    if args.std_log:
        print(
            f"[opal] dataset_windows={agent._dataset.valid_starts.shape[0]} "
    ...
    run_offline_pretraining(
        ...
        checkpoint_dir=checkpoint_dir,
        checkpoint_freq=args.checkpoint_freq,
        save_filename="opal_offline_pretrained.pt",
...
registry.register("opal", OPALArgs, run_opal)
