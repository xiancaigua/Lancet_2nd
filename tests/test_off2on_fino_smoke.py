...
def _make_agent(**overrides) -> Off2OnFINO:
    kwargs = dict(
        env=_FakeEnv(),
        device="cpu",
        buffer_device="cpu",
    ...
    return Off2OnFINO(**kwargs)


def _load_offline(agent: Off2OnFINO, tmp_path) -> None:
    path = tmp_path / "fino.h5"
    _write_h5_dataset(path, num_traj=6, steps_per_traj=20)
    load_h5_dataset_to_replay_buffer(agent.replay_buffer, str(path))


def test_off2on_fino_defaults_match_fino_recipe():
    agent = _make_agent()
    assert agent.noise_scale == 0.1
    assert agent.beta == 10.0
    # ACTION_DIM=2 -> min(10, (2+1)//2) == 1
    assert agent.num_samples == 1


def test_off2on_fino_offline_training_produces_finite_losses(tmp_path):
    agent = _make_agent()
...
def test_off2on_fino_switch_to_online_and_learn(tmp_path):
    # num_samples=4 so the Boltzmann-weighted categorical over K>1 candidates
    # is actually exercised (with ACTION_DIM=2 the default resolves to 1).
...
def test_off2on_fino_checkpoint_roundtrip(tmp_path):
    agent = _make_agent()
    _load_offline(agent, tmp_path)
    agent.train(5)
    ckpt = agent.save(tmp_path / "fino.pt")
    ...
    for (n1, p1), (n2, p2) in zip(
        ...
        assert n1 == n2
        assert torch.allclose(p1, p2)
    assert agent2.noise_scale == agent.noise_scale
    assert agent2.beta == agent.beta
