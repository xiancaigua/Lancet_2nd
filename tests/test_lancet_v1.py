"""Focused, download-free LancetV1 v1 regression tests."""
...
def _agent(*, use_lancet: bool = True) -> LancetV1:
    return LancetV1(
        env=DummyVecEnv(),
        device="cpu",
        buffer_device="cpu",
...
def test_residual_network_shape():
    net = ResidualQNetworkV1(4, 2, [8, 8])
...
def test_checkpoint_round_trip_and_online_switch(tmp_path):
    ...
    before = {
        k: v.detach().clone() for k, v in agent.residual_network.state_dict().items()
    }
...
def test_u_variation_requires_an_approved_formula():
    with pytest.raises(ValueError, match="U-variation"):
        LancetV1(
            env=DummyVecEnv(), device="cpu", buffer_device="cpu", lambda_u_variation=1.0
        )
...
def test_lancet_registry_discovery():
    ...
    registry.discover()
    assert "lancet_v1" in registry.entries()
