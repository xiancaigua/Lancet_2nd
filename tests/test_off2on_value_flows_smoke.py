...
class _FakeEnv:
    ...
    copy of tests/test_off2on_floq_smoke.py's ``_FakeEnv`` (no chunking)."""
...
def _make_agent(**overrides) -> Off2OnValueFlows:
    kwargs = dict(
        env=_FakeEnv(),
        device="cpu",
        buffer_device="cpu",
    ...
    return Off2OnValueFlows(**kwargs)


def _load_offline(agent: Off2OnValueFlows, tmp_path) -> None:
    path = tmp_path / "value_flows.h5"
    _write_h5_dataset(path, num_traj=6, steps_per_traj=20)
    load_h5_dataset_to_replay_buffer(agent.replay_buffer, str(path))


def test_off2on_value_flows_defaults_match_recipe():
    agent = _make_agent()
    assert agent.q_agg == "mean"
    assert agent.min_reward == -1.0
    assert agent.max_reward == 0.0
    assert agent.ret_agg == "mean"


def test_off2on_value_flows_offline_training_produces_finite_losses(tmp_path):
    agent = _make_agent()
...
def test_off2on_value_flows_switch_to_online_and_learn(tmp_path):
    agent = _make_agent()
    _load_offline(agent, tmp_path)
    agent.train(5)
    assert agent.policy_extraction == "rs"
    assert agent.policy.policy_extraction == "rs"
    agent.switch_to_online_mode(online_replay_mode="mixed", offline_data_ratio=0.5)
    assert agent.policy_extraction == "rpg"
    assert agent.policy.policy_extraction == "rpg"
...
def test_off2on_value_flows_checkpoint_roundtrip_after_online_switch(tmp_path):
    """`policy_extraction` is checkpoint metadata (`_checkpoint_metadata`)
    and is mutated at the online switch
    (`_apply_online_regularizer_override`) -- verify saving post-switch
    ("rpg") and loading into a default-constructed agent ("rs") round-trips
    the network weights without raising. Like every other constructor-only
    hyperparameter in this checkpoint format (metadata is validated for
    compatibility, not replayed into the loading agent's Python attributes),
    `policy_extraction` itself is not restored by `load()` -- the loaded
    agent keeps its own constructed value."""
    agent = _make_agent()
    _load_offline(agent, tmp_path)
    agent.train(5)
    agent.switch_to_online_mode(online_replay_mode="append")
    assert agent.policy.policy_extraction == "rpg"
    ckpt = agent.save(tmp_path / "value_flows_online.pt")

    loaded = _make_agent()
    assert loaded.policy_extraction == "rs"
    loaded.load(ckpt)
    assert loaded.policy_extraction == "rs"
    assert loaded.policy.policy_extraction == "rs"
    for (n1, p1), (n2, p2) in zip(
        agent.policy.critic_flows.named_parameters(),
        loaded.policy.critic_flows.named_parameters(),
    ):
        assert n1 == n2
        assert torch.allclose(p1, p2)


def test_off2on_value_flows_checkpoint_roundtrip(tmp_path):
    agent = _make_agent()
    _load_offline(agent, tmp_path)
    agent.train(5)
    ckpt = agent.save(tmp_path / "value_flows.pt")

    agent2 = _make_agent()
    agent2.load(ckpt)
    for (n1, p1), (n2, p2) in zip(
        agent.policy.critic_flows.named_parameters(),
        agent2.policy.critic_flows.named_parameters(),
    ):
        assert n1 == n2
        assert torch.allclose(p1, p2)
    assert agent2.min_reward == agent.min_reward
    assert agent2.ret_agg == agent.ret_agg
