...
set -euo pipefail
cd "$(dirname "$0")/.."
python -m pytest -q tests/test_lancet.py
python examples/train_off2on.py lancet \
  --config configs/off2on/lancet_antmaze_medium_play_smoke.yaml --print-config >/dev/null
echo "Lancet unit/registry smoke passed."
